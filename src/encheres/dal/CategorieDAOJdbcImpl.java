...
public class CategorieDAOJdbcImpl implements CategorieDAO {
	...
	private static final String SELECT_CATEGORIE_ALL=" SELECT " + 
			"	c.no_categorie as noCategorie," + 
			"	c.libelle," +
			" FROM" + 
			"	CATEGORIES c";
	private static final String SELECT_CATEGORIE_NO=" SELECT " + 
			"	c.no_categorie as noCategorie," + 
			"	c.libelle," +
			" FROM" + 
			"	CATEGORIES c" +
			" WHERE c.no_categorie=?";
	private static final String SELECT_CATEGORIE_LIBELLE=" SELECT " + 
			"	c.no_categorie as noCategorie," + 
			"	c.libelle," +
			" FROM" + 
			"	CATEGORIES c" +
	...
	public List<Categorie> selectAll() throws BusinessException {
		...
		try(Connection cnx = ConnectionProvider.getConnection())
		{
			PreparedStatement pstmt = cnx.prepareStatement(SELECT_CATEGORIE_ALL);
	...
	public Categorie select(int noCategorie) throws BusinessException {
		...
		try(Connection cnx = ConnectionProvider.getConnection()) {
			PreparedStatement pstmt = cnx.prepareStatement(SELECT_CATEGORIE_NO, PreparedStatement.RETURN_GENERATED_KEYS);
	...
	public Categorie select(String libelle) throws BusinessException {
		...
		Categorie categorie = new Categorie();
		try(Connection cnx = ConnectionProvider.getConnection()) {
			PreparedStatement pstmt = cnx.prepareStatement(SELECT_CATEGORIE_LIBELLE, PreparedStatement.RETURN_GENERATED_KEYS);
